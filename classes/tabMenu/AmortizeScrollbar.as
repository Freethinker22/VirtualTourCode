package classes.tabMenu // This class takes a DisplayObject from Amortization and adds a scroll bar to it
{
	...
	import classes.ScrollbarConstruct;
	
	public class AmortizeScrollbar extends ScrollbarConstruct
	{
		...
		private var _contentTop:Number; // Top limit of _targetObject
		private var _contentBottom:Number; // Bottom limit of _targetObject
		private var _dragHandleTop:Number; // Top limit of _dragHandle
		private var _dragHandleBottom:Number; // Bottom limit of _dragHandle
		...
		private var _downwardOffset:Number; // Distance from top of ObjectContainer to top of targetMC
		...
		public function AmortizeScrollbar(theme:Theme, target:DisplayObject, trackWidth:Number, trackHeight:Number, downwardOffset:Number = 0, moveAmt:Number = 15, maskX:Number = 0, maskY:Number = 0):void
		{
			// Set argument vars
			_theme = theme;
		...
		{	
			...
			_range = _track.height - _dragHandle.height; // still drags too far, range and scale issue with really tall object
			...
			_scrollbar.x = _targetObject.x + _targetObject.width + 8; // Horizontal placement of Scrollbar, 8 is padding
			_scrollbar.y = _targetObject.y + _downwardOffset + 7; // Vertical placement of Scrollbar, 7 is padding
		...
		private function setMask():void
		{
			...
			_mask.graphics.drawRect(_maskX, _maskY, _targetObject.width, (_track.height + _upScrollBtn.height + _downScrollBtn.height));
		...
		private function moveScroll(e:MouseEvent):void 
		{
			...
			// Move _dragHandle vertically with with _changeInY
